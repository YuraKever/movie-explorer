...
Тот же путь, но срез становится «продуктовым».

- [ ] `MovieCard` + `MovieGrid`, постеры через `next/image`
- [ ] Адаптивная сетка
- [ ] Тёмная тема (`next-themes`) + `Header` + `ThemeToggle`
- [ ] Типы `Movie`, серверный клиент `lib/tmdb.ts`
...
- [ ] `/movie/[id]` (RSC): постер, описание, рейтинг, жанры
- [ ] Навигация с карточки
- [ ] `generateMetadata` (SEO/превью ссылок)
